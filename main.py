...
ti.init(arch=ti.gpu)
resolution = width, height = vec2(800, 800)
...
class PyShader:
    ...
    def render(self, time: float):
        ...
        for frag_coord in ti.grouped(self.screen_field):
            ...
            if value < -0.45:
                self.screen_field[frag_coord.x, resolution.y - frag_coord.y] = vec3(128, 197, 222)
                continue
            if value < -0.35:
                self.screen_field[frag_coord.x, resolution.y - frag_coord.y] = vec3(248, 240, 164)
                continue
            if value < 0.35:
                self.screen_field[frag_coord.x, resolution.y - frag_coord.y] = vec3(126, 200, 80)
                continue
            if value < 0.53:
                self.screen_field[frag_coord.x, resolution.y - frag_coord.y] = vec3(200, 200, 200)
                continue
            else:
...
@ti.data_oriented
class ComputeShader:
    def __init__(self, app, compute_area, seed):
        self.app = app
    ...
    def render(self, time: float):
        ...
        for frag_coord in ti.grouped(self.screen_field):
            ...
            random_seed = random(self.seed, self.seed)

            noise = 1 * snoise((p.x + random_seed) * 1, (p.y + random_seed) * 1) + \
                    0.5 * snoise((p.x + random_seed) * 2, (p.y + random_seed) * 2) + \
                    0.25 * snoise((p.x + random_seed) * 4, (p.y + random_seed) * 4) + \
                    0.125 * snoise((p.x + random_seed) * 8, (p.y + random_seed) * 8) + \
                    0.0625 * snoise((p.x + random_seed) * 16, (p.y + random_seed) * 16) + \
                    0.03125 * snoise((p.x + random_seed) * 32, (p.y + random_seed) * 32) + \
                    0.015625 * snoise((p.x + random_seed) * 64, (p.y + random_seed) * 64) + \
                    0.0078125 * snoise((p.x + random_seed) * 128, (p.y + random_seed) * 128)
...
class App:
    def __init__(self):
        ...
        self.compute_shader = ComputeShader(self, [width, height], 12)
